...
import yaml
import os
from llm_vlm_planner.utils.logical_twin import LogicalTwin
    ...
    def plan(self, task, plan, outcomes):
        ...
        for plan_step in plan:
            ...
            while not success:
                ...
                primitive = re.search(r'\[\s*.*?\s*\]', plan_step_primitives, re.DOTALL)
                """
                if primitive:
                    obj_list_str = primitive.group(0)
                    obj_list = ast.literal_eval(obj_list_str)  
                    print(f"Primitives: {obj_list}")
                else:
